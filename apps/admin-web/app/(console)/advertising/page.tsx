...
import {
  Icon, Badge, StatCard, SecHead, It, Tabs, Modal, ReasonList, Bar, fmtINR,
} from '@/components/ui';

const SLOT_INFO: Record<string, { sub: string; rate: number }> = {
...
export default function AdvertisingPage() {
  ...
  return (
    <div className="page page-wide">
      <SecHead eyebrow="ADVERTISING · FEATURED PLACEMENTS" title={<>Paid <It>spotlight</It>.</>}>
        <button className="btn btn-dark" onClick={() => setCreateOpen(true)}><Icon name="plus" size={16} />New placement</button>
      </SecHead>
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 24 }}>
        ...
        <StatCard label="Slot fill rate" value={stats.fillRate ?? '—'} unit="%" meta={`${stats.openSlots ?? 0} slots open in ${stats.openCity || 'Pune'}`} color="var(--info)" />
      </div>
          ...
          <thead>
            <tr>
              {['Campaign', 'Slot', 'City', 'Flight', 'Budget', 'Spent', 'Clicks', 'Status', ''].map((h, i) => (
                <th key={i} className={['Budget', 'Spent', 'Clicks'].includes(h) ? 'num-col' : ''}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {(d?.ads || []).map((a: any) => (
              <tr key={a.adId}>
                <td>
                  ...
                  <div className="muted" style={{ fontSize: 11.5 }}>{a.owner}</div>
                </td>
                <td><span className="badge blue" style={{ fontSize: 11 }}><Icon name="megaphone" size={12} />{a.slot}</span></td>
                <td className="muted">{a.city}</td>
                <td className="muted" style={{ fontSize: 12 }}>{a.start.slice(5)} → {a.end.slice(5)}</td>
                <td className="num-col strong">{fmtINR(a.budget)}</td>
                <td className="num-col"><span className="muted">{fmtINR(a.spent)}</span></td>
                ...
                <td className="num-col">
                  {a.status === 'review' ? (
                    ...
                    <button className="btn btn-sm btn-ghost" onClick={() => setViewing(a)}><Icon name="eye" size={14} /></button>
                  )}
                </td>
              </tr>
            ))}
            {!(d?.ads || []).length && (
              <tr><td colSpan={9} className="muted" style={{ padding: 24, textAlign: 'center' }}>No placements in this view.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      {createOpen && <PlacementModal onClose={() => setCreateOpen(false)} onDone={() => { setCreateOpen(false); refetch(); }} />}
      ...
      {viewing && (
        <Modal
          ...
          width={460}
          foot={<button className="btn" onClick={() => setViewing(null)}>Close</button>}
        >
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 18 }}>
            <div className="card card-pad" style={{ padding: 16 }}>
              ...
              <div className="num" style={{ fontSize: 20, marginTop: 6 }}>{(viewing.impressions || 0).toLocaleString('en-IN')}</div>
            </div>
            <div className="card card-pad" style={{ padding: 16 }}>
              <div className="eyebrow">CLICKS</div>
              <div className="num" style={{ fontSize: 20, marginTop: 6 }}>{(viewing.clicks || 0).toLocaleString('en-IN')}</div>
            </div>
            <div className="card card-pad" style={{ padding: 16 }}>
          ...
          <div className="eyebrow" style={{ marginBottom: 8 }}>SPEND PACING</div>
...
function PlacementModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const toast = useToast();
    ...
    <Modal
      ...
      foot={
        <>
          ...
          >
            <Icon name="megaphone" size={16} />Create placement
          </button>
        </>
      }
    >
      <div className="field">
        ...
        <select value={venueId} onChange={(e) => setVenueId(e.target.value)}>
          ...
          {venues.map((v: any) => <option key={v.venueId} value={v.venueId}>{v.venueName} — {v.city}</option>)}
        </select>
      </div>
      <div className="field">
        ...
        <ReasonList
          options={slots.map(([name, info]) => ({
            title: name, sub: info.sub,
            right: <span className="mono" style={{ fontSize: 12, fontWeight: 600 }}>₹{info.rate.toLocaleString('en-IN')}/mo</span>,
          }))}
          value={slotIdx}
          onChange={(i) => { setSlotIdx(i); setBudget(slots[i][1].rate); }}
        />
      </div>
      <div className="row gap12">
        <div className="field" style={{ flex: 1 }}><label>Start date</label><input type="date" value={start} onChange={(e) => setStart(e.target.value)} /></div>
        <div className="field" style={{ flex: 1 }}><label>End date</label><input type="date" value={end} onChange={(e) => setEnd(e.target.value)} /></div>
      </div>
      ...
      <div className="row between" style={{ marginTop: 16, padding: '12px 14px', background: 'var(--info-soft)', borderRadius: 10 }}>
        <span style={{ fontSize: 13, fontWeight: 500 }}>{slotName} · {venue?.city || 'pick a venue'}</span>
        <span className="num" style={{ fontSize: 18, color: 'var(--info)' }}>{fmtINR(budget)}</span>
      </div>
    </Modal>
  );
}
