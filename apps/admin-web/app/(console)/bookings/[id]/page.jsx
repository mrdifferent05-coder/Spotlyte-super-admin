...
import { Icon, Avatar, Badge, Panel, KV, Modal, Mono, StepTimeline, ReasonList, HeadBar, Back, fmtINR } from '@/components/ui';

const REFUND_REASONS = [
  'Venue cancelled the slot',
  'Facility unavailable / unsafe',
  'Customer cancelled in policy window',
  'Double booking',
  ...
  return (
    <div className="page">
      ...
      <HeadBar
        ...
        sub={
          <>
            <a className="lk" onClick={() => b.customerId && router.push(`/users/${b.customerId}`)}>
              {b.customerName}
            </a>{' '}
            ·{' '}
            <a className="lk" onClick={() => b.venueId && router.push(`/venues/${b.venueId}`)}>
              {b.venueName}
            </a>{' '}
            · {b.courtName}
          </>
        ...
        actions={
          <>
            {refundable && (
              <button
                className="btn"
                style={{ color: 'var(--danger)', borderColor: 'color-mix(in oklab, var(--danger) 35%, var(--line))' }}
          ...
          <div className="card card-pad">
            ...
            <div className="col gap10">
              <div className="row between">
                <span className="muted">Slot total</span>
                <span className="strong" style={{ color: 'var(--ink)', fontWeight: 500 }}>
                  {fmtINR(br.amount, { full: true })}
                </span>
              </div>
              <div className="row between">
                <span className="muted">Platform fee ({br.feePct}%)</span>
                <span style={{ color: 'var(--danger)' }}>−{fmtINR(br.fee, { full: true })}</span>
              </div>
              {d.refund && (
                <div className="row between">
                  <span className="muted">Refunded ({d.refund.pct}%)</span>
                  <span style={{ color: 'var(--danger)' }}>−{fmtINR(d.refund.amount, { full: true })}</span>
                </div>
              )}
              ...
              <div className="row between">
                ...
                <span className="num" style={{ fontSize: 18 }}>
                  {fmtINR(br.ownerEarns, { full: true })}
                </span>
              </div>
            </div>
          </div>
          {d.refund && (
            <div className="card card-pad" style={{ background: 'var(--danger-soft)', borderColor: 'color-mix(in oklab, var(--danger) 22%, transparent)' }}>
              <div className="row gap10">
                ...
                <div>
                  <b style={{ fontSize: 13 }}>
                    Refund issued · {fmtINR(d.refund.amount, { full: true })} ({d.refund.pct}%)
                  </b>
                  <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>
                    {d.refund.reason}
                    {d.refund.note ? ` — ${d.refund.note}` : ''}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card card-pad">
            ...
            <KV
              items={[
                [
                  'Customer',
                  <a key="c" className="lk" onClick={() => b.customerId && router.push(`/users/${b.customerId}`)}>
                    {b.customerName}
                  </a>,
                ],
                [
                  'Venue',
                  <a key="v" className="lk" onClick={() => b.venueId && router.push(`/venues/${b.venueId}`)}>
                    {b.venueName}
                  </a>,
                ],
                ['Court', b.courtName],
        ...
        <Modal
          ...
          foot={
            <>
              ...
              >
                <Icon name="wallet" size={16} />
                Refund {fmtINR(refundAmt, { full: true })}
              </button>
            </>
          }
        >
          ...
          <div className="field">
            ...
            <div className="row between" style={{ marginTop: 12, padding: '12px 14px', background: 'var(--danger-soft)', borderRadius: 10 }}>
              <span style={{ fontSize: 13, fontWeight: 600 }}>
                {refPct}% of {fmtINR(b.amount, { full: true })}
              </span>
              <span className="num" style={{ fontSize: 19, color: 'var(--danger)' }}>
                {fmtINR(refundAmt, { full: true })}
              </span>
            </div>
          </div>
