...
export default function BookingsPage() {
  ...
  const stats = d?.stats || {};
  const rows = (d?.bookings || []).map((b) => ({ ...b, id: b.bookingId, customer: b.customerName, venue: b.venueName }));
  const [sorted, sort, toggle] = useSort(rows);

  return (
    <div className="page page-wide">
      <SecHead
        eyebrow={`BOOKINGS · ${counts.all ?? '—'} · 30D`}
        title={
          <>
            Every slot, <It>booked</It>.
          </>
        }
      >
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 24 }}>
        ...
        <StatCard
          ...
          unit="%"
          delta={stats.cancellationRate != null ? -1 : null}
          meta={stats.cxlMeta}
          color="var(--brand)"
        />
        ...
        <table className="tbl">
          <thead>
            <tr>
              ...
              <Th k="createdAt" sort={sort} toggle={toggle}>
                Date · Time
              </Th>
              <Th k="method" sort={sort} toggle={toggle}>
                Payment
              </Th>
              ...
              <tr key={b.bookingId} className="clickable" onClick={() => router.push(`/bookings/${b.bookingId}`)}>
                ...
                <td>
                  <div className="who">
                    ...
                    <span className="nm">{b.customer}</span>
                  </div>
                </td>
                <td>
