...
import { Icon, Avatar, Badge, Urgency, Panel, KV, Modal, StepTimeline, ReasonList, HeadBar, Back, avColor, fmtINR } from '@/components/ui';
  ...
  return (
    <div className="page">
      ...
      <HeadBar
        ...
        sub={
          <>
            <a className="lk" onClick={() => cd.customer?.userId && router.push(`/users/${cd.customer.userId}`)}>
              {x.customerName}
            </a>{' '}
            vs{' '}
            <a className="lk" onClick={() => x.venueId && router.push(`/venues/${x.venueId}`)}>
              {x.venueName}
            </a>{' '}
            · booking{' '}
            <a className="lk mono" style={{ fontSize: 12 }} onClick={() => x.bookingId && router.push(`/bookings/${x.bookingId}`)}>
              #{x.bookingId}
            </a>
          </>
        }
      ...
      <div className="grid" style={{ gridTemplateColumns: '1.5fr 1fr', alignItems: 'start' }}>
            ...
            <div className="col gap16">
              {d.thread.map((t, i) => (
                <div key={i} className="row gap12" style={{ alignItems: 'flex-start' }}>
                  <Avatar name={t.senderName} color={t.senderRole === 'admin' ? 'var(--ink)' : t.senderRole === 'owner' ? 'var(--warn)' : avColor(t.senderName)} />

                  <div
                    className="fill"
                    style={{
                      background: t.senderRole === 'admin' ? 'var(--brand-tint)' : 'var(--wash)',
                  ...
                  >
                    ...
                    <p style={{ fontSize: 13.5, marginTop: 5, lineHeight: 1.5, color: 'var(--ink-2)' }}>{t.senderRole === 'admin' ? t.message : `“${t.message}”`}</p>
                  </div>
                </div>
              ))}
              {x.status !== 'resolved' && (
        ...
        <div className="col gap16">
          <div className="card card-pad">
            ...
            {d.escrowHeld ? (
              <>
                <div className="num" style={{ fontSize: 30, color: 'var(--warn)' }}>
                  {fmtINR(amount, { full: true })}
                </div>
              ...
              <>
                <div className="num" style={{ fontSize: 30, color: 'var(--muted-2)' }}>
                  ₹0
                </div>
                <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
                  Escrow released · {d.resolution?.label || 'resolved'}
                </div>
              </>
            )}
          </div>
          <div className="card card-pad">
            ...
            <KV
              items={[
                [
                  'Customer',
                  <a key="c" className="lk" onClick={() => cd.customer?.userId && router.push(`/users/${cd.customer.userId}`)}>
                    {cd.customer?.name}
                  </a>,
                ],
                [
                  'Owner',
                  <a key="o" className="lk" onClick={() => cd.owner?.ownerId && router.push(`/owners/${cd.owner.ownerId}`)}>
                    {cd.owner?.name}
                  </a>,
                ],
                [
                  'Venue',
                  <a key="v" className="lk" onClick={() => cd.venue?.venueId && router.push(`/venues/${cd.venue.venueId}`)}>
                    {cd.venue?.name}
                  </a>,
                ],
                ['Slot', cd.slot],
      ...
      {modal && (
        ...
        >
          ...
          <ReasonList options={RESOLUTIONS.map((r) => ({ title: r.title, sub: r.sub }))} value={resolution} onChange={setResolution} />
