...
export default function InvoiceDocPage() {
  ...
  if (!d) return <div className="page"><div className="muted" style={{ padding: 40 }}>Loading invoice…</div></div>;
  const inv = d.invoice;
  ...
  return (
    <div className="page">
      <div className="row between no-print" style={{ marginBottom: 18 }}>
        ...
        <button className="btn btn-dark" onClick={() => window.print()}><Icon name="download" size={16} />Print / PDF</button>
      </div>
      ...
      <div className="inv-doc">
        <div className="row between" style={{ alignItems: 'flex-start', marginBottom: 30 }}>
          <div>
            <div className="row gap8" style={{ alignItems: 'baseline' }}>
              <span className="brand-word" style={{ fontSize: 22 }}>spotlyte.</span>
          ...
          <div className="right">
            ...
            <div className="mono" style={{ fontSize: 16, fontWeight: 600, marginTop: 4 }}>{inv.invoiceId}</div>
            <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>Issued {inv.issued}</div>
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 28 }}>
          <div>
            <div className="eyebrow" style={{ marginBottom: 8 }}>BILLED TO</div>
            ...
            <div className="muted" style={{ fontSize: 12.5, marginTop: 4, lineHeight: 1.6 }}>
              ...
              {buyer.city}
              {buyer.gstin && <><br />GSTIN <Mono>{buyer.gstin}</Mono></>}
            </div>
          </div>
          <div className="right">
            <div className="eyebrow" style={{ marginBottom: 8 }}>DETAILS</div>
            <div className="muted" style={{ fontSize: 12.5, lineHeight: 1.8 }}>
              Type · <b style={{ color: 'var(--ink)' }}>{inv.type}</b><br />
              Period · <b style={{ color: 'var(--ink)' }}>{inv.period}</b><br />
        ...
        <table className="tbl" style={{ marginBottom: 8 }}>
          <thead>
            <tr><th>Description</th><th className="num-col">Amount</th></tr>
          </thead>
          <tbody>
        ...
        <div className="row between" style={{ padding: '16px 16px 0', borderTop: '1px solid var(--line)' }}>
          ...
          <span className="num" style={{ fontSize: 22 }}>{fmtINR(inv.total, { full: true })}</span>
        </div>
