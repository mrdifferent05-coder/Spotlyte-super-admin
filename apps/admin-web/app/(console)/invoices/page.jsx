...
import { Icon, Avatar, Badge, StatCard, SecHead, It, Seg, useSort, Th, Modal, fmtINR } from '@/components/ui';
...
export default function InvoicesPage() {
  ...
  const stats = d?.stats || {};
  const rows = (d?.invoices || []).map((i) => ({ ...i, id: i.invoiceId }));
  ...
  return (
    <div className="page page-wide">
      <SecHead
        eyebrow="INVOICES · TAX & BILLING"
        title={
          <>
            Every <It>rupee</It>, documented.
          </>
        }
      >
        ...
        <button className="btn btn-dark" onClick={() => setCreate(true)}>
        ...
        {aud === 'owner' ? (
          <>
            ...
            <StatCard
              ...
              unit={collected.unit}
              delta={7}
              meta={`${stats.collectionRate ?? 0}% collection rate`}
              color="var(--brand)"
            />
      ...
      <div className="row between wrap gap12" style={{ marginBottom: 18 }}>
        <Seg
          light
          options={[
            { value: 'owner', label: 'To owners' },
            { value: 'user', label: 'To users' },
          ]}
          value={aud}
          onChange={(v) => {
            setAud(v);
        ...
        <table className="tbl">
          <thead>
            <tr>
              ...
              <Th k="party" sort={sort} toggle={toggle}>
                {aud === 'owner' ? 'Owner' : 'User'}
              </Th>
              <Th k="type" sort={sort} toggle={toggle}>
                Type
              </Th>
              <Th k="period" sort={sort} toggle={toggle}>
                Period
              </Th>
              <Th k="issued" sort={sort} toggle={toggle}>
                Issued
              </Th>
              <Th k="total" sort={sort} toggle={toggle} num>
                Total (incl. GST)
              </Th>
          ...
          <tbody>
            {sorted.length ? (
              sorted.map((i) => (
                <tr key={i.invoiceId}>
                  <td>
                    <span className="ref">{i.invoiceId}</span>
                  </td>
                  <td>
                    <div className="who">
                      <Avatar color={i.color} init={i.initials} />
                      <div>
                        <div className="nm">{i.party}</div>
...
function NewInvoiceModal({ onClose, onDone }) {
  const toast = useToast();
  const [aud, setAud] = useState('owner');
    ...
    >
      <div className="field">
        ...
        <Seg
          ...
          options={[
            ...
            { value: 'user', label: 'To user' },
          ]}
          value={aud}
          onChange={(v) => {
            setAud(v);
            setPartyId('');
            setType(v === 'owner' ? 'Subscription' : 'Booking receipt');
          }}
          style={{ width: '100%' }}
        />
      </div>
      ...
      <div className="row gap12">
        <div className="field" style={{ flex: 1 }}>
          ...
          <select value={type} onChange={(e) => setType(e.target.value)}>
            {(aud === 'owner' ? ['Subscription', 'Commission'] : ['Booking receipt']).map((t) => (
      ...
      >
        <span style={{ fontSize: 13 }}>
          ₹{amount.toLocaleString('en-IN')} + GST ₹{gstAmt.toLocaleString('en-IN')}
        </span>
        <span className="num" style={{ fontSize: 18, color: 'var(--brand-deep)' }}>
          ₹{(amount + gstAmt).toLocaleString('en-IN')}
        </span>
      </div>
    </Modal>
  );
