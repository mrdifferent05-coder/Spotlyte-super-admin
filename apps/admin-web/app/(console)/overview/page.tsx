...
import {
  Icon, Sparkline, StatCard, MiniStat, Panel, SecHead, It, Badge, Urgency, Avatar, Seg, Bar, VenueThumb, fmtINR,
} from '@/components/ui';

const TONE_COLOR: Record<string, string> = { brand: 'var(--brand)', info: 'var(--info)', danger: 'var(--danger)', warn: 'var(--warn)' };
...
export default function OverviewPage() {
  ...
  if (!d) return <div className="page page-wide"><div className="muted" style={{ padding: 40 }}>Loading command center…</div></div>;

  const exportCSV = () =>
    downloadCSV(`spotlyte-overview-${range}`, [
      ...d.heroKpis.map((k: any) => ({ section: 'KPI', label: k.label, value: k.value + (k.unit || ''), meta: k.meta })),
      ...d.sportMix.map((s: any) => ({ section: 'Sport mix', label: s.sport, value: `${s.percent}%`, meta: fmtINR(s.revenue) })),
      ...d.topVenues.map((v: any) => ({ section: 'Top venue', label: v.venueName, value: fmtINR(v.revenue30d), meta: `${v.occupancy}% occ · ${v.avgRating}★` })),
    ]);
  ...
  return (
    <div className="page page-wide">
      ...
      <div className="sec-head" style={{ marginTop: 36 }}>
        <div className="l">
          <div className="eyebrow" style={{ marginBottom: 7 }}>{d.dateLine}</div>
        ...
        <div className="row gap8">
          ...
          <button className="btn" onClick={exportCSV}><Icon name="download" size={16} />Export</button>
        </div>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1.55fr 1fr', alignItems: 'start' }}>
        <Panel
          ...
          pad={false}
          action={<button className="btn btn-sm" onClick={() => router.push('/owners')}>Open queue<Icon name="arrowRight" size={15} /></button>}
        >
          <div style={{ padding: '4px 10px 10px' }}>
            {d.approvalQueue.length === 0 && <div className="muted" style={{ padding: 16, fontSize: 13 }}>Queue clear — nothing waiting on you.</div>}
            {d.approvalQueue.map((q: any, i: number) => (
              <div
                key={i}
                className="row gap12"
              ...
              >
                ...
                <div className="fill">
                  <div className="row gap8">
                    ...
                    <span className="badge gray" style={{ height: 20, fontSize: 10.5 }}>{q.kind}</span>
                  </div>
                  <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{q.who} · {q.meta}</div>
                </div>
                <Badge status={q.status} />
                <span className="muted mono" style={{ fontSize: 11, width: 64, textAlign: 'right' }}>{q.submittedAgo || '—'}</span>
            ...
            {d.sportMix.map((s: any) => (
              <div key={s.sport}>
                <div className="row between" style={{ marginBottom: 6 }}>
                  ...
                  <span className="muted" style={{ fontSize: 12.5 }}>{fmtINR(s.revenue)} · {s.percent}%</span>
                </div>
                <div className="bar"><i style={{ width: s.percent + '%', background: s.colorVar }} /></div>
              </div>
            ))}
          </div>
        </Panel>
      ...
      <div style={{ marginTop: 36 }}>
        <Panel
          ...
          pad={false}
          action={<button className="btn btn-sm" onClick={() => router.push('/disputes')}>Open queue<Icon name="arrowRight" size={15} /></button>}
        >
              ...
              <thead>
                <tr>
                  {['Ref', 'Urgency', 'Customer', 'Slot · Venue', 'Reason', 'Amount', 'Opened', ''].map((h, i) => (
                    <th key={i} className={h === 'Amount' ? 'num-col' : ''}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {d.openDisputes.map((x: any) => (
                  <tr key={x.disputeId} className="clickable" onClick={() => router.push(`/disputes/${x.disputeId}`)}>
                    <td><span className="ref">#{x.disputeId}</span></td>
                    ...
                    <td>
                      ...
                      <div className="muted" style={{ fontSize: 12 }}>{x.venueName}</div>
                    </td>
                    <td>{x.reason}</td>
                    <td className="num-col strong">{fmtINR(x.amount, { full: true })}</td>
                    <td className="muted">{x.openedLabel}</td>
                    <td className="num-col">
                      <button className="btn btn-sm" onClick={(e) => { e.stopPropagation(); router.push(`/disputes/${x.disputeId}`); }}>
      ...
      <div style={{ marginTop: 36 }}>
        <Panel
          ...
          pad={false}
          action={<button className="btn btn-sm" onClick={() => router.push('/venues')}>All venues<Icon name="arrowRight" size={15} /></button>}
        >
          <div style={{ padding: '8px 10px 6px', overflowX: 'auto' }}>
            <table className="tbl">
              <thead>
                <tr>
                  {['Venue', 'Owner', 'GMV · 30D', 'Trend', 'Bookings', 'Occupancy', 'Rating', ''].map((h, i) => (
                    <th key={i} className={['GMV · 30D', 'Bookings'].includes(h) ? 'num-col' : ''}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {d.topVenues.map((v: any) => (
                  <tr key={v.venueId} className="clickable" onClick={() => router.push(`/venues/${v.venueId}`)}>
                    ...
                    <td>
                      <div className="row gap8">
                        ...
                        <span className="muted" style={{ fontSize: 12 }}>{v.occupancy}%</span>
                      </div>
                    </td>
                    <td>
                      <span className="row gap6">
                        ...
                        <span className="muted" style={{ fontSize: 12 }}>· {v.reviewCount}</span>
                      </span>
                    </td>
