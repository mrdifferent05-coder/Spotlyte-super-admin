...
import {
  OWNER_DETAIL, APPROVE_KYC, REJECT_KYC, REQUEST_DOCS, SUSPEND_OWNER, REINSTATE_OWNER, REFRESH_NAV,
} from '@/lib/gql';
import { useToast } from '@/lib/toast';
import {
  Icon, Avatar, Badge, Panel, KV, Modal, Mono, Sparkline, Pipeline, ActivityLog, RequestDocsModal,
  ReasonList, Stat, HeadBar, Back, VenueThumb, fmtINR,
} from '@/components/ui';
...
const CXL_TONE: Record<string, string> = { Flexible: 'green', Moderate: 'amber', Strict: 'red' };

export default function OwnerDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const toast = useToast();
  const [modal, setModal] = useState<null | 'docs' | 'reject' | 'approve' | 'viewer'>(null);
  ...
  if (!d) return <div className="page"><div className="muted" style={{ padding: 40 }}>Loading owner…</div></div>;
  const o = d.owner;
  ...
  return (
    <div className="page">
      ...
      <HeadBar
        ...
        title={o.biz}
        badges={<><Badge status={o.kyc} /><span className="badge gray" style={{ fontSize: 11 }}>{o.ownerId}</span></>}
        ...
        actions={
          pending ? (
            <>
              ...
              {o.kyc === 'review' && (
                <button className="btn" onClick={() => setModal('docs')}><Icon name="mail" size={16} />Request docs</button>
              )}
              <button className="btn btn-primary" onClick={() => setModal('approve')}><Icon name="check" size={16} />Approve KYC</button>
            </>
          ) : (
            <>
              <button className="btn" onClick={() => toast(`Message sent to ${o.name}`)}><Icon name="mail" size={16} />Message</button>
              {o.kyc === 'verified' ? (
                <button
                  className="btn" style={{ color: 'var(--danger)' }}
                  onClick={async () => { const r = await suspend({ variables: { ownerId: o.ownerId } }); toast(r.data?.adminSuspendOwner?.message || 'Owner suspended'); }}
                >
                ...
                <button
                  ...
                  onClick={async () => { const r = await reinstate({ variables: { ownerId: o.ownerId } }); toast(r.data?.adminReinstateOwner?.message || 'Owner reinstated'); }}
                >
                  <Icon name="refresh" size={16} />Reinstate
                </button>
              )}
            </>
          )
      ...
      <div className="grid" style={{ gridTemplateColumns: '1.6fr 1fr', alignItems: 'start' }}>
          ...
          <Panel eyebrow="KYC DOCUMENTS" title="Verify the paperwork.">
            <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {d.documents.map((doc: any, i: number) => (
                <div key={i} className="doc" style={{ cursor: 'pointer' }} onClick={() => { setViewerDoc(doc); setModal('viewer'); }}>
                  <div className="ico"><Icon name="doc" size={18} /></div>
                  <div className="fill">
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{doc.name}</div>
                    <div className="mono muted" style={{ fontSize: 11, marginTop: 2 }}>{doc.ref || '—'}</div>
                  </div>
                  {doc.status === 'verified' ? (
                    <span className="badge green" style={{ fontSize: 10.5 }}><Icon name="check" size={12} />Verified</span>
                  ) : doc.status === 'missing' ? (
                    <span className="badge red" style={{ fontSize: 10.5 }}>Missing</span>
                  ) : (
                    <span className="badge amber" style={{ fontSize: 10.5 }}>Review</span>
                  )}
                </div>
              ))}
            </div>
          ...
          {d.venues.length > 0 && (
            <Panel eyebrow={`VENUES · ${d.venues.length}`} title="What they operate." pad={false}>
              <div style={{ padding: '6px 10px 10px' }}>
                {d.venues.map((v: any) => (
                  <div
                    key={v.venueId} className="row gap12"
                  ...
                  >
                    ...
                    <div className="fill">
                      <div className="strong" style={{ fontSize: 14, color: 'var(--ink)', fontWeight: 500 }}>{v.venueName}</div>
                      <div className="muted" style={{ fontSize: 12 }}>{v.courtCount} courts · {v.city}</div>
                    </div>
                    <Badge status={v.venueStatus} />
                    <span className="num" style={{ fontSize: 15, width: 70, textAlign: 'right' }}>{fmtINR(v.revenue30d)}</span>
        ...
        <div className="col gap16">
          ...
          <div className="card card-pad">
            <div className="eyebrow" style={{ marginBottom: 14 }}>LEGAL ENTITY</div>
            <KV items={[
          ...
          <div className="card card-pad">
            <div className="row between" style={{ marginBottom: 14 }}>
              ...
              {d.bank?.verified && <span className="badge green" style={{ fontSize: 10.5 }}><Icon name="check" size={12} />Verified</span>}
            </div>
            <KV items={[
          ...
          <div className="card card-pad">
            <div className="eyebrow" style={{ marginBottom: 14 }}>SUBSCRIPTION PLAN</div>
            <div className="row between" style={{ alignItems: 'flex-start' }}>
              <div>
                <div className="row gap8" style={{ alignItems: 'center' }}>
                  <span className="num" style={{ fontSize: 19 }}>{plan.name}</span>
                  <span className="badge green" style={{ fontSize: 10.5 }}>Active</span>
                </div>
                <div className="muted" style={{ fontSize: 12.5, marginTop: 4 }}>{plan.commissionLine}</div>
              </div>
              <div className="right">
                <div className="num" style={{ fontSize: 17 }}>{plan.priceMo ? `₹${Number(plan.priceMo).toLocaleString('en-IN')}` : 'Free'}</div>
                <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>per mo</div>
              </div>
            </div>
          ...
          <div className="card card-pad">
            <div className="eyebrow" style={{ marginBottom: 12 }}>CANCELLATION POLICY</div>
          ...
          <div className="card card-pad">
            <div className="eyebrow" style={{ marginBottom: 12 }}>GMV TREND · 14 DAYS</div>
        ...
        <Modal
          ...
          foot={
            <>
              ...
              >
                <Icon name="check" size={16} />Confirm approval
              </button>
            </>
          }
        >
          <div className="col gap10">
            <div className="row gap10" style={{ padding: 14, background: 'var(--brand-tint)', borderRadius: 12, border: '1px solid color-mix(in oklab, var(--brand) 20%, transparent)' }}>
      ...
      {modal === 'viewer' && viewerDoc && (
        <Modal
          ...
          onClose={() => { setModal(null); setViewerDoc(null); }}
          width={520}
          foot={<button className="btn" onClick={() => { setModal(null); setViewerDoc(null); }}>Close</button>}
        >
          {viewerDoc.fileUrl ? (
            ...
            <div style={{ height: 260, borderRadius: 12, border: '1px dashed var(--line)', display: 'grid', placeItems: 'center', background: 'var(--wash)' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div className="mono" style={{ fontSize: 12, marginTop: 4 }}>{viewerDoc.ref}</div>
              </div>
            </div>
          )}
        </Modal>
