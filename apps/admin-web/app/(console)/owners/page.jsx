...
import { Icon, Avatar, Badge, StatCard, SecHead, It, Tabs, Toolbar, useSort, Th, Modal, fmtINR } from '@/components/ui';
  ...
  return (
    <div className="page page-wide">
      ...
      >
        ...
        <button className="btn btn-dark" onClick={() => setInvite(true)}>
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 24 }}>
        ...
        <StatCard
          label="Owner GMV · 30D"
          value={fmtINR(stats.gmv30d).replace(/ (Cr|L)$/, '')}
          unit={/ (Cr|L)$/.test(fmtINR(stats.gmv30d)) ? fmtINR(stats.gmv30d).match(/ (Cr|L)$/)[1] : undefined}
          delta={stats.gmvDelta}
          meta={`Top: ${stats.topBiz ?? '—'}`}
        />
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        <table className="tbl">
          <thead>
            <tr>
              <Th k="biz" sort={sort} toggle={toggle}>
                Owner / Business
              </Th>
              <Th k="city" sort={sort} toggle={toggle}>
                City
              </Th>
              <Th k="kyc" sort={sort} toggle={toggle}>
                KYC
              </Th>
              <Th k="venues" sort={sort} toggle={toggle} num>
                Venues
              </Th>
              ...
              <Th k="disputes" sort={sort} toggle={toggle} num>
                Disputes
              </Th>
              <th />
            </tr>
          ...
          <tbody>
            {sorted.map((o) => (
              <tr key={o.ownerId} className="clickable" onClick={() => router.push(`/owners/${o.ownerId}`)}>
                ...
                <td className="num-col">
                  {o.disputes ? (
                    <span className={'badge ' + (o.disputes > 2 ? 'red' : 'amber')} style={{ fontSize: 11 }}>
                      {o.disputes}
                    </span>
                  ) : (
...
function InviteOwnerModal({ onClose, onDone }) {
  ...
  return (
        ...
        <>
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (data?.adminInviteOwner?.success) {
                toast('Invite sent');
                onDone();
    ...
    >
      ...
      <div className="field">
        ...
        <input value={form.biz} onChange={(e) => set('biz', e.target.value)} placeholder="e.g. Northside Turf Pvt Ltd" />
      </div>
      <div className="row gap12">
        <div className="field" style={{ flex: 1.4 }}>
          <label>Email</label>
          <input type="email" value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="owner@business.in" />
        </div>
