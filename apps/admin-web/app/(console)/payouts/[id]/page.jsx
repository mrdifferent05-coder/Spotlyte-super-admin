...
import { Icon, Avatar, Badge, Panel, Modal, StepTimeline, Stat, HeadBar, Back, fmtINR } from '@/components/ui';
...
export default function PayoutDetailPage() {
  ...
  const breakdown = useMemo(() => {
    ...
    const cats = {};
    const add = (key, name, price, color, b) => {
          ...
          p.status === 'queued' ? (
            <>
              <button
                className="btn"
                onClick={async () => {
                  const r = await hold({ variables: { payoutId: p.payoutId } });
                  toast(r.data?.adminHoldPayout?.message || 'Payout put on hold');
                }}
              >
              ...
              <button
                ...
                onClick={async () => {
                  const r = await approve({ variables: { payoutId: p.payoutId } });
            ...
            <button
              ...
              onClick={async () => {
                const r = await retry({ variables: { payoutId: p.payoutId } });
          ...
          <Panel eyebrow="SETTLEMENT BREAKDOWN" title="How it adds up." pad={false}>
                ...
                <thead>
                  <tr>
                    {['Slot type', 'Rate', 'Slots', 'Gross', `Fee`].map((h, i) => (
                      <th key={i} className={['Slots', 'Gross', 'Fee'].includes(h) ? 'num-col' : ''} style={i === 0 ? { paddingLeft: 24 } : undefined}>
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {!breakdown.length && (
                    <tr>
                      <td colSpan={5} className="muted" style={{ padding: 20, textAlign: 'center' }}>
                        No linked transactions on this run.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div style={{ padding: '14px 24px 18px', borderTop: '1px solid var(--line)' }}>
              ...
              <div className="row between" style={{ padding: '12px 0 0', marginTop: 6, borderTop: '1px solid var(--line)' }}>
                ...
                <span className="num" style={{ fontSize: 20, color: 'var(--ok)' }}>
                  {fmtINR(p.amount, { full: true })}
                </span>
              </div>
            </div>
          </Panel>
                ...
                {!d.transactions.length && (
                  <tr>
                    <td colSpan={8} className="muted" style={{ padding: 20, textAlign: 'center' }}>
                      No linked bookings on this payout.
                    </td>
                  </tr>
                )}
              </tbody>
              ...
              <tbody>
                {d.history.map((h) => (
                  <tr key={h.payoutId} className="clickable" onClick={() => router.push(`/payouts/${h.payoutId}`)}>
                    <td className="strong" style={{ paddingLeft: 24 }}>
                      #{h.payoutId}
                    </td>
                    <td className="muted">{h.period}</td>
                    <td>
                      <span className="ref">{h.utr || '—'}</span>
                    </td>
                ...
                {!d.history.length && (
                  <tr>
                    <td colSpan={6} className="muted" style={{ padding: 20, textAlign: 'center' }}>
                      No past settlements for this owner.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        >
          <div className="row gap10" style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--info-soft)', borderRadius: 10 }}>
            ...
            <div style={{ fontSize: 13 }}>
              Initiate the bank transfer of <b>{fmtINR(p.amount, { full: true })}</b> to <b>{p.bank}</b>, then enter the UTR / reference number to mark this payout
              settled.
            </div>
          </div>
