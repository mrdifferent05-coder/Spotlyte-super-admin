...
import {
  Icon, Avatar, Badge, StatCard, SecHead, It, Tabs, useSort, Th, fmtINR,
} from '@/components/ui';
...
export default function PayoutsPage() {
  ...
  const stats = d?.stats || {};
  const rows = (d?.payouts || []).map((p: any) => ({ ...p, id: p.payoutId }));
  ...
  return (
    <div className="page page-wide">
      <SecHead eyebrow={`PAYOUTS · NEXT RUN ${d?.nextRunLabel || '—'}`} title={<>Money <It>moving</It>.</>}>
        <button
          className="btn"
          onClick={() => downloadCSV('spotlyte-payout-statement', rows.map((p: any) => ({
            id: p.payoutId, owner: p.owner, biz: p.biz, period: p.period, gross: p.gross, fee: p.fee, tds: p.tds, net: p.amount, status: p.status, utr: p.utr || '',
          })))}
        >
          <Icon name="download" size={16} />Statement
        </button>
        <button
        ...
        >
          <Icon name="bolt" size={16} />Run payouts
        </button>
      </SecHead>

      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 24 }}>
        <StatCard label="Queued for payout" value={fmtINR(stats.queuedTotal)} meta={`${counts.queued ?? 0} queued · ${counts.processing ?? 0} processing`} color="var(--info)" />
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        <table className="tbl">
          <thead>
            <tr>
              <Th k="id" sort={sort} toggle={toggle}>Payout</Th>
              <Th k="owner" sort={sort} toggle={toggle}>Owner</Th>
              <Th k="period" sort={sort} toggle={toggle}>Period</Th>
              <Th k="gross" sort={sort} toggle={toggle} num>Gross</Th>
              <Th k="fee" sort={sort} toggle={toggle} num>Fee</Th>
              <Th k="amount" sort={sort} toggle={toggle} num>Net payout</Th>
          ...
          <tbody>
            {sorted.map((p: any) => (
              <tr key={p.payoutId} className="clickable" onClick={() => router.push(`/payouts/${p.payoutId}`)}>
                ...
                <td><div className="who"><Avatar color={p.color} init={p.initials} /><div><div className="nm">{p.owner}</div><div className="sub">{p.biz}</div></div></div></td>
                ...
                <td className="num-col">
                  {p.status === 'queued' ? (
                    ...
                    <span className="muted" style={{ fontSize: 12 }}>{p.settles}</span>
                  )}
                </td>
              </tr>
            ))}
            {!sorted.length && (
              <tr><td colSpan={8} className="muted" style={{ padding: 24, textAlign: 'center' }}>No payouts in this view. Hit “Run payouts” to queue settlements.</td></tr>
            )}
          </tbody>
        </table>
      </div>
