...
import {
  Icon, Badge, StatCard, SecHead, It, Toolbar, Modal, Seg, Bar, fmtINR,
} from '@/components/ui';
...
export default function PromosPage() {
  ...
  return (
    <div className="page page-wide">
      <SecHead eyebrow={`PROMO CODES · ${stats.active ?? '—'} ACTIVE`} title={<>Levers for <It>growth</It>.</>}>
        <button className="btn btn-dark" onClick={() => setCreateOpen(true)}><Icon name="plus" size={16} />Create code</button>
      </SecHead>
          ...
          <thead>
            <tr>
              {['Code', 'Discount', 'Scope', 'Redemptions', 'Expires', 'Status', ''].map((h, i) => (
                <th key={i} className={h === 'Redemptions' ? 'num-col' : ''}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((p: any) => (
              <tr key={p.promoId}>
                <td>
                  <span className="mono" style={{ fontWeight: 600, fontSize: 13, color: 'var(--ink)' }}>{p.code}</span>
                  <div className="muted" style={{ fontSize: 11.5 }}>{p.label}</div>
                </td>
                <td><span className="badge brand-chip">{p.kind === 'pct' ? `${p.value}% off` : `₹${p.value} off`}</span></td>
                <td className="muted">{p.scope}</td>
                <td className="num-col">
                  <div className="strong">{p.used.toLocaleString('en-IN')} / {p.cap.toLocaleString('en-IN')}</div>
                ...
                <td className="num-col">
                  <button className="btn btn-sm btn-ghost" onClick={() => setEditing(p)}><Icon name="pen" size={14} /></button>
                </td>
              </tr>
            ))}
            {!rows.length && (
              <tr><td colSpan={7} className="muted" style={{ padding: 24, textAlign: 'center' }}>No promo codes match.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      {createOpen && <PromoModal onClose={() => setCreateOpen(false)} onDone={() => { setCreateOpen(false); refetch(); }} />}
      {editing && <PromoModal promo={editing} onClose={() => setEditing(null)} onDone={() => { setEditing(null); refetch(); }} />}
    </div>
  );
}

function PromoModal({ promo, onClose, onDone }: { promo?: any; onClose: () => void; onDone: () => void }) {
  const toast = useToast();
  const isEdit = !!promo;
  ...
  const doUpdate = async (patch: any, msg?: string) => {
      ...
      foot={
        <>
          {isEdit && (
            <>
              {promo.status === 'paused' ? (
                <button className="btn" onClick={() => doUpdate({ status: 'active' })}><Icon name="play" size={15} />Resume</button>
              ) : (
                <button className="btn" onClick={() => doUpdate({ status: 'paused' })}><Icon name="pause" size={15} />Pause</button>
              )}
              <button
                className="btn"
                onClick={async () => {
                  const dup = `${form.code}2`.slice(0, 12);
                  const { data } = await createPromo({ variables: { input: { code: dup, kind: form.kind, value: +form.value, scope: form.scope, cap: +form.cap, expires: form.expires, label: form.label || undefined } } });
          ...
          <button
            ...
            onClick={async () => {
              if (isEdit) {
                ...
                const { data } = await createPromo({ variables: { input: { code: form.code, kind: form.kind, value: +form.value, scope: form.scope, cap: +form.cap, expires: form.expires, label: form.label || undefined } } });
          ...
          >
            <Icon name="check" size={16} />{isEdit ? 'Save changes' : 'Create & activate'}
          </button>
        </>
      }
    >
      ...
      <div className="row gap12">
        <div className="field" style={{ flex: 1 }}>
          ...
          <Seg
            light
            options={[{ value: 'pct', label: '% off' }, { value: 'flat', label: '₹ flat' }]}
            value={form.kind}
      ...
      <div className="row gap10" style={{ marginTop: 16, padding: '12px 14px', background: 'var(--brand-tint)', borderRadius: 10, border: '1px solid color-mix(in oklab, var(--brand) 20%, transparent)' }}>
        <span className="mono" style={{ fontWeight: 600, fontSize: 14, color: 'var(--brand-deep)' }}>{form.code || 'YOURCODE'}</span>
