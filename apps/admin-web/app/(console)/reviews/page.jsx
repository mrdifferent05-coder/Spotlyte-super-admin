...
import { Icon, Avatar, Badge, Stars, StatCard, SecHead, It, Tabs, Modal } from '@/components/ui';
...
function AutoflagModal({ onClose }) {
  const toast = useToast();
  ...
  return (
    ...
    >
      {RULES.map((r) => (
        <div key={r.key} className={'checkrow' + (rules[r.key] ? ' on' : '')} onClick={() => setLocal({ ...rules, [r.key]: !rules[r.key] })}>
