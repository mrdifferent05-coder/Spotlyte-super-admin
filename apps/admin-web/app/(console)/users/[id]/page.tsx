...
import {
  Icon, Avatar, Badge, Panel, KV, Mono, Sparkline, Stat, HeadBar, Back, VenueThumb, fmtINR,
} from '@/components/ui';
...
export default function UserDetailPage() {
  ...
  if (!d) return <div className="page"><div className="muted" style={{ padding: 40 }}>Loading user…</div></div>;
  const u = d.user;
  ...
  return (
    <div className="page">
      ...
      <HeadBar
        ...
        title={u.name}
        badges={<><Badge status={u.status} /><span className="badge gray" style={{ fontSize: 11 }}>{u.userId}</span></>}
        sub={`${u.city} · joined ${u.joined} · last seen ${u.lastSeen}`}
        actions={
          <>
            <button className="btn" onClick={() => toast(`Message sent to ${u.name}`)}><Icon name="mail" size={16} />Message</button>
            {u.status === 'banned' ? (
              <button
                className="btn btn-primary"
                onClick={async () => { const r = await reinstate({ variables: { userId: u.userId } }); toast(r.data?.adminReinstateUser?.message || 'User reinstated'); }}
              >
              ...
              <button
                ...
                onClick={async () => { const r = await ban({ variables: { userId: u.userId } }); toast(r.data?.adminBanUser?.message || 'User banned'); }}
              >
                <Icon name="ban" size={16} />Ban user
              </button>
            )}
          </>
        }
              ...
              {d.bookings.length ? d.bookings.map((b: any) => (
                ...
                >
                  ...
                  <div className="fill">
                    ...
                    <div className="muted" style={{ fontSize: 12 }}>{b.courtName} · {b.date}</div>
                  </div>
            ...
            <div className="card card-pad" style={{ background: 'var(--danger-soft)', borderColor: 'color-mix(in oklab, var(--danger) 22%, transparent)' }}>
              <div className="row gap10">
                ...
                <div>
                  ...
                  <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{s.flaggedNote || 'Fraud & abuse signals'}</div>
                </div>
              </div>
            </div>
          )}
          <div className="card card-pad">
            <div className="eyebrow" style={{ marginBottom: 12 }}>SPEND TREND · 14 DAYS</div>
