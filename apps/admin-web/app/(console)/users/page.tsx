...
export default function UsersPage() {
  ...
  const stats = d?.stats || {};
  const rows = (d?.users || []).map((u: any) => ({ ...u, id: u.userId }));
  const [sorted, sort, toggle] = useSort(rows);

  return (
    <div className="page page-wide">
      <SecHead eyebrow={`USERS · ${stats.registered ?? '—'} REGISTERED`} title={<>The people who <It>play</It>.</>}>
        <button
          className="btn"
          onClick={() => downloadCSV('spotlyte-users', rows.map((u: any) => ({
            id: u.userId, name: u.name, email: u.email, city: u.city, joined: u.joined, bookings: u.bookings, spend: u.spend, status: u.status,
          })))}
        >
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        <table className="tbl">
          <thead>
            <tr>
              <Th k="name" sort={sort} toggle={toggle}>User</Th>
              <Th k="city" sort={sort} toggle={toggle}>City</Th>
              <Th k="joined" sort={sort} toggle={toggle}>Joined</Th>
              <Th k="bookings" sort={sort} toggle={toggle} num>Bookings</Th>
              <Th k="spend" sort={sort} toggle={toggle} num>Lifetime spend</Th>
              <Th k="lastSeen" sort={sort} toggle={toggle}>Last seen</Th>
          ...
          <tbody>
            ...
            {!sorted.length && (
              <tr><td colSpan={7} className="muted" style={{ padding: 24, textAlign: 'center' }}>No users in this view.</td></tr>
            )}
          </tbody>
        </table>
      </div>
