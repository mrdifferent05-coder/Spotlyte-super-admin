...
import {
  VENUE_DETAIL, APPROVE_VENUE, REJECT_VENUE, FEATURE_VENUE, UNFEATURE_VENUE, PAUSE_VENUE, RESUME_VENUE, REREVIEW_VENUE, REFRESH_NAV,
} from '@/lib/gql';
import { useToast } from '@/lib/toast';
import {
  Icon, Avatar, Badge, Urgency, Panel, KV, Modal, Mono, Sparkline, Tabs, ActivityLog, ReasonList,
  Stat, HeadBar, Back, EmptyState, fmtINR, avColor,
} from '@/components/ui';
...
const AMENITY_ICON: Record<string, string> = {
  Floodlights: 'bolt', Parking: 'mapPin', 'Changing rooms': 'users', 'Drinking water': 'wallet', Washrooms: 'building',
  'Seating / gallery': 'venues', 'Equipment rental': 'bookings', 'First aid': 'shield', CCTV: 'eye', Cafeteria: 'star',
};
const FEATURE_SLOTS = [
...
export default function VenueDetailPage() {
  ...
  const [modal, setModal] = useState<null | 'reject' | 'feature'>(null);
  ...
  if (!d) return <div className="page"><div className="muted" style={{ padding: 40 }}>Loading venue…</div></div>;
  const v = d.venue;
  ...
  const run = async (fn: any, vars: any, fallback: string, key: string) => {
    ...
    <div className="page">
      ...
      <HeadBar
        ...
        badges={
          <>
            <Badge status={status} />
            {v.featured && <span className="badge lime-chip"><Icon name="crown" size={13} />Featured</span>}
            <span className="badge gray" style={{ fontSize: 11 }}>{v.venueId}</span>
          </>
        }
        sub={
          <>
            {v.sport} ·{' '}
            <a className="lk" onClick={() => d.ownerId && router.push(`/owners/${d.ownerId}`)}>{v.ownerName}</a>
        ...
        actions={
          pending ? (
            <>
              <button className="btn btn-danger" onClick={() => setModal('reject')}><Icon name="x" size={16} />Reject</button>
              <button className="btn btn-primary" onClick={() => run(approve, { venueId: v.venueId }, 'Venue approved · live on marketplace', 'adminApproveVenueV2')}>
                <Icon name="check" size={16} />Approve venue
              </button>
            </>
          ) : status === 'live' ? (
            <>
              {v.featured ? (
                <button className="btn btn-dark" onClick={() => run(unfeature, { venueId: v.venueId }, 'Removed from featured', 'adminUnfeatureVenue')}>
                  <Icon name="crown" size={16} />Unfeature
                </button>
              ) : (
                <button className="btn" onClick={() => setModal('feature')}><Icon name="crown" size={16} />Feature venue</button>
              )}
              <button className="btn" onClick={() => run(pause, { venueId: v.venueId }, 'Venue paused', 'adminPauseVenue')}>
                <Icon name="pause" size={16} />Pause listing
              </button>
            </>
          ) : status === 'paused' ? (
            ...
            <button className="btn btn-primary" onClick={() => run(reReview, { venueId: v.venueId }, 'Back in review queue', 'adminReReviewVenue')}>
              <Icon name="refresh" size={16} />Re-review
            </button>
          )
        }
      />
      ...
      {modal === 'reject' && (
        ...
        >
          <p className="muted" style={{ fontSize: 13.5, marginBottom: 16 }}>The owner is notified and can resubmit after fixes.</p>
      ...
      {modal === 'feature' && (
        ...
        >
          <div className="field">
            ...
            <ReasonList
              options={FEATURE_SLOTS.map((s) => ({
                ...
                right: <span className="mono" style={{ fontSize: 12, fontWeight: 600 }}>₹{s.rate.toLocaleString('en-IN')}/mo</span>,
              }))}
              value={featSlot}
          ...
          <div className="row between" style={{ marginTop: 16, padding: '12px 14px', background: 'var(--lime)', borderRadius: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 500, color: '#0E0E0C' }}>{FEATURE_SLOTS[featSlot].title} · {featMonths} mo</span>
            <span className="num" style={{ fontSize: 18, color: '#0E0E0C' }}>{fmtINR(FEATURE_SLOTS[featSlot].rate * featMonths)}</span>
          </div>
        </Modal>
      )}
    </div>
...
function OverviewTab({ d, v, c, router, setTab }: any) {
  return (
    <div className="grid" style={{ gridTemplateColumns: '1.4fr 1fr 1fr', alignItems: 'start' }}>
      <div className="col gap16">
        <Panel eyebrow={`PHOTOS · ${d.photos.length}`} title="How it shows up.">
          <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
            {d.photos.map((_p: string, i: number) => (
              <div key={i} style={{ aspectRatio: '1', borderRadius: 10, background: `linear-gradient(${135 + i * 25}deg, ${c}, color-mix(in oklab, ${c} 50%, #000))`, position: 'relative' }}>
                {i === 0 && <span className="badge solid-dark" style={{ position: 'absolute', bottom: 6, left: 6, fontSize: 10 }}>Cover</span>}
              </div>
            ))}
          </div>
        </Panel>
        <Panel
          eyebrow="RECENT BOOKINGS" title="Live activity." pad={false}
          action={<button className="btn btn-sm" onClick={() => setTab('bookings')}>View all<Icon name="arrowRight" size={14} /></button>}
        >
          <div style={{ padding: '4px 10px 10px' }}>
            {d.recentBookings.length ? d.recentBookings.map((b: any) => (
              <div
                key={b.bookingId} className="row gap12"
                style={{ padding: '11px 14px', borderRadius: 11, cursor: 'pointer' }}
                onClick={() => router.push(`/bookings/${b.bookingId}`)}
                onMouseEnter={(e: any) => (e.currentTarget.style.background = 'var(--wash)')}
              ...
              >
                ...
                <div className="fill">
                  ...
                  <div className="muted" style={{ fontSize: 12 }}>{b.courtName} · {b.date} · {b.timeRange}</div>
                </div>
                <Badge status={b.status} />
                <span className="num" style={{ fontSize: 14, width: 62, textAlign: 'right' }}>{fmtINR(b.amount, { full: true })}</span>
              </div>
            )) : <div className="muted" style={{ padding: 16, fontSize: 13 }}>No bookings yet.</div>}
          </div>
        </Panel>
      </div>
      ...
      <div className="col gap16">
        <div className="card card-pad">
          <div className="eyebrow" style={{ marginBottom: 14 }}>GAMES OFFERED</div>
          <div className="chips">
            {d.games.map((g: string) => (
              <span key={g} className="badge brand-chip" style={{ height: 28 }}><Icon name="venues" size={13} />{g}</span>
            ))}
          </div>
        </div>
        <Panel eyebrow={`COURTS · ${d.courts.length}`} title="What can be booked." pad={false}>
          <div style={{ padding: '4px 8px 8px' }}>
            {d.courts.map((ct: any, i: number) => (
              <div key={i} className="row gap12" style={{ padding: '11px 14px', borderBottom: i < d.courts.length - 1 ? '1px solid var(--line-soft)' : 'none' }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, display: 'grid', placeItems: 'center', background: 'var(--wash)', border: '1px solid var(--line)', color: 'var(--muted)', flexShrink: 0 }}>
                  <Icon name="venues" size={16} />
                </div>
                <div className="fill">
                  <div style={{ fontSize: 13.5, fontWeight: 500, color: 'var(--ink)' }}>{ct.courtName} · {ct.sport}</div>
                  <div className="muted" style={{ fontSize: 12, marginTop: 1 }}>{ct.surface} · {ct.size}</div>
                </div>
                <span className="badge green" style={{ fontSize: 10.5 }}><span className="dot" />Active</span>
              </div>
            ))}
          </div>
        </Panel>
        <div className="card card-pad">
          <div className="eyebrow" style={{ marginBottom: 14 }}>AMENITIES</div>
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {d.amenities.map((a: string) => (
              <div key={a} className="row gap8" style={{ fontSize: 13, color: 'var(--ink-2)' }}>
                <div style={{ width: 26, height: 26, borderRadius: 7, display: 'grid', placeItems: 'center', background: 'var(--brand-tint)', color: 'var(--brand)', flexShrink: 0 }}>
      ...
      <div className="col gap16">
        <div className="card card-pad">
          <div className="eyebrow" style={{ marginBottom: 14 }}>LISTING DETAILS</div>
          <KV items={[
            ['Owner', <a key="o" className="lk" onClick={() => d.listing?.owner?.ownerId && router.push(`/owners/${d.listing.owner.ownerId}`)}>{d.listing?.owner?.name}</a>],
        ...
        <div className="card card-pad">
          <div className="eyebrow" style={{ marginBottom: 12 }}>PRICING · PER HOUR</div>
          <KV items={[
        ...
        <div className="card card-pad">
          <div className="eyebrow" style={{ marginBottom: 12 }}>GMV TREND</div>
...
// ── Availability tab ──────────────────────────────────
function AvailabilityTab({ d }: any) {
  const legend = (
    <div className="row gap12" style={{ fontSize: 11.5 }}>
      {([['Booked', 'var(--brand)'], ['Open', 'var(--paper)'], ['Blocked', 'var(--line)']] as [string, string][]).map((l, i) => (
  ...
  const times = d.courts[0]?.slots?.map((s: any) => s.time) || [];
  return (
        ...
        <table className="tbl" style={{ minWidth: 720 }}>
          <thead>
            <tr>
              ...
              {times.map((t: string) => <th key={t} className="num-col" style={{ textAlign: 'center' }}>{t}</th>)}
            </tr>
          </thead>
          <tbody>
            {d.courts.map((ct: any, ci: number) => (
              <tr key={ci}>
                <td style={{ paddingLeft: 24 }}>
                  <div className="strong">{ct.courtName}</div>
                  <div className="muted" style={{ fontSize: 11.5 }}>{ct.sport}</div>
                </td>
                {ct.slots.map((s: any, si: number) => {
                  const sty =
                    s.state === 'booked' ? { background: 'var(--brand)', color: '#fff' }
                    : s.state === 'blocked' ? { background: 'var(--line)', color: 'var(--muted)' }
...
// ── Bookings tab ──────────────────────────────────────
function BookingsTab({ d, router }: any) {
  return (
    <Panel eyebrow={`ALL BOOKINGS · ${d.bookings.length}`} title="Every slot here." pad={false}>
      <div style={{ overflowX: 'auto' }}>
        <table className="tbl">
          ...
          <tbody>
            {d.bookings.length ? d.bookings.map((b: any) => (
              ...
              <tr><td colSpan={7} className="muted" style={{ padding: 24, textAlign: 'center' }}>No bookings yet for this venue.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
// ── Disputes tab ──────────────────────────────────────
function DisputesTab({ d, router }: any) {
          ...
          <tbody>
            {d.disputes.map((x: any) => (
              <tr key={x.disputeId} className="clickable" onClick={() => router.push(`/disputes/${x.disputeId}`)}>
                <td style={{ paddingLeft: 24 }}><span className="ref">#{x.disputeId}</span></td>
                <td><Urgency level={x.urgency} /></td>
                <td><div className="who"><Avatar color={x.color} init={x.avatarInitials} /><span className="nm">{x.customerName}</span></div></td>
                <td className="muted" style={{ fontSize: 12 }}>{x.slotLabel}</td>
                <td>{x.reason}</td>
...
// ── Payouts tab ───────────────────────────────────────
function PayoutsTab({ d, ps, router }: any) {
  return (
    <div className="col gap16">
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
        ...
        <div className="card card-pad" style={{ background: 'var(--info-soft)', borderColor: 'color-mix(in oklab, var(--info) 22%, transparent)' }}>
          <div className="eyebrow" style={{ color: 'var(--info)' }}>UPCOMING PAYOUT</div>
          <div className="num" style={{ fontSize: 24, marginTop: 8, color: 'var(--info)' }}>{fmtINR(ps.upcomingAmount)}</div>
          <div className="muted" style={{ fontSize: 12, marginTop: 5 }}>{ps.upcomingLabel}</div>
        </div>
            ...
            <tbody>
              {d.payouts.length ? d.payouts.map((p: any) => (
                <tr key={p.payoutId} className="clickable" onClick={() => router.push(`/payouts/${p.payoutId}`)}>
                  <td style={{ paddingLeft: 24 }}><span className="ref">#{p.payoutId}</span></td>
                  <td className="muted">{p.period}</td>
                  <td className="num-col muted">{fmtINR(p.gross)}</td>
                  <td className="num-col" style={{ color: 'var(--danger)' }}>−{fmtINR(p.fee)}</td>
                ...
                <tr><td colSpan={7} className="muted" style={{ padding: 24, textAlign: 'center' }}>No payouts yet for this venue.</td></tr>
              )}
            </tbody>
          </table>
        </div>
