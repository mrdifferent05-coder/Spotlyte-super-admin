...
type NavItem = { route: string; label: string; icon: string; countKey?: string; tone?: 'warn' | 'alert' };
...
const KIND_ICON: Record<string, string> = { owner: 'owners', venue: 'venues', booking: 'bookings', user: 'users' };
const KIND_LABEL: Record<string, string> = { owner: 'Owners', venue: 'Venues', booking: 'Bookings', user: 'Users' };
    ...
    <div className="app">
      <aside className="aside">
        <div className="brand-row">
          ...
          <button className="collapse-btn" title="Notifications"><Icon name="bell" size={16} /></button>
        </div>
        <nav className="nav">
          {NAV.map((g) => (
      ...
      <main className="main" ref={mainRef}>
        <header className="topbar">
          ...
          <button className="btn icon-btn" title="Notifications"><Icon name="bell" size={18} /></button>
        </header>
        {children}
      </main>
...
function UserMenu({ onClose }: { onClose: () => void }) {
  ...
  const [density, setDensity] = useState<string>(() =>
    typeof document !== 'undefined' ? document.documentElement.getAttribute('data-density') || 'regular' : 'regular'
  );
  ...
  useEffect(() => {
    const onDoc = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest('.user-menu, .user-chip')) onClose();
    };
...
function CommandPalette({ onClose }: { onClose: () => void }) {
  ...
  const grouped = ['owner', 'venue', 'booking', 'user']
    .map((k) => ({ kind: k, items: hits.filter((h: any) => h.kind === k) }))
    ...
    <>
      ...
      <div className="palette">
        <div className="pal-input">
          ...
          />
          <kbd style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', border: '1px solid var(--line)', borderRadius: 6, padding: '1px 5px', background: 'var(--wash)' }}>ESC</kbd>
        </div>
              ...
              {g.items.map((h: any) => {
                ...
                return (
                  <div key={h.kind + h.id} className={'pal-item' + (idx === sel ? ' sel' : '')} onClick={() => go(h)} onMouseEnter={() => setSel(idx)}>
                    <div style={{ width: 30, height: 30, borderRadius: 8, display: 'grid', placeItems: 'center', background: 'var(--wash)', border: '1px solid var(--line)', color: 'var(--muted)', flexShrink: 0 }}>
                      <Icon name={KIND_ICON[h.kind]} size={15} />
                    </div>
                    <div className="fill">
                      <div style={{ fontSize: 13.5, fontWeight: 500, color: 'var(--ink)' }}>{h.title}</div>
                      {h.sub && <div className="muted" style={{ fontSize: 11.5 }}>{h.sub}</div>}
                    </div>
